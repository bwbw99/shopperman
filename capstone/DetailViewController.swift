...
class DetailViewController:UIViewController,CLLocationManagerDelegate{
    ...
    @IBAction func SelectButtonTapped(_ sender: Any) {
        print(DetailViewController.ids)
        print(id)
        var flag : Int = 0
        for i in 0..<DetailViewController.ids.count{
            if id == DetailViewController.ids[i]{
                flag += 1
            }
        }
        if flag > 0{
            print("a")
            ErrorMessage.text = "이미 담은 의뢰입니다"
        }
        else if DetailViewController.ids.count > 5{
            print("b")
            ErrorMessage.text = "최대 6개의 의뢰만 담을 수 있습니다"
        }
        else{
            DetailViewController.ids.append(id)
            print(DetailViewController.ids)
            NotificationCenter.default.post(name: NSNotification.Name("notiData"), object: id)
