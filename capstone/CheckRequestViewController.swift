//
//  CheckRequestViewController.swift
//  capstone
//
...
class CheckRequestViewController:UIViewController{
    ...
    private func loadPage(){
        ...
        let url = URL(string: "http://52.78.16.203:8080/post/get/list/publish")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "req_1"{
            guard let vc = segue.destination as? MyRequestDetail else {return}
            vc.id = Int(id_1.text!)!
            vc.token = token
        }
