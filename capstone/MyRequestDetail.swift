...
class MyRequestDetail:UIViewController, CLLocationManagerDelegate{
    ...
    @IBAction func CompleteButtonTapped(_ sender: UIButton) {
        ...
        let url = URL(string: "http://52.78.16.203:8080/delivery/complete/confirm?postId=\(id)")
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            DispatchQueue.main.async {
                do{
                    ...
                    print("배달 완료 확인 (complete/request) 결과: \(result)")
                }catch {
